...
public class AuthService {
    ...
    public TokenDto login(UserLoginRequestDto userLoginRequestDto) {  // 로그인 기능. (로그인 경우에만 AuthenticationProvider를 통해 아이디와 비밀번호 일치 체킹을 진행한다.)
        ...
        // UsernamePasswordAuthenticationToken은 input 받은 로그인아이디(사용자명username)과 비밀번호(password)를 갖는 인증 토큰 객체임.
        ...
        // 그 과정에서 'DB에서 가져온 UserDetails 객체'정보와 'input되어 생성시킨 UsernamePasswordAuthenticationToken 인증토큰객체'정보를 비교해서, 로그인계정아이디가 일치하는지 체킹한다.
        // 또한 비밀번호 일치 과정도 일어나는데, 이는 SecurityConfig에서 등록했던 PasswordEncoder Bean 객체를 통해 Spring Security 내부적으로 이루어진다. 따라서 조회된 User에는 패스워드가 해당 PasswordEncoder를 통해 암호화된 상태로 저장되어 있어야한다.
        // ----------
        ...
        // 즉, 여기서 실제로 사용자에 대한 자세한 검증이 이루어지며, 로그인아이디와 비밀번호에 대한 일치 체킹은 AuthenticationProvider에서 진행하는 역할이다.
